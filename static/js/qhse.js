...
/** Ligne « Aucune donnée » (ne doit pas être comptée comme une ligne métier). */
function qhseIsTablePlaceholderRow(tr) {
  return tr && tr.hasAttribute('data-qhse-placeholder');
}
...
function qhseInitTableSearch(inputId, tableId, countId) {
  ...
  function updateCount() {
    const visible = Array.from(tbody.querySelectorAll('tr')).filter(r => {
      if (qhseIsTablePlaceholderRow(r)) return false;
  ...
  input.addEventListener('input', () => {
    ...
    Array.from(tbody.querySelectorAll('tr')).forEach(row => {
      if (qhseIsTablePlaceholderRow(row)) {
...
function qhseInitPagination(tableId, pageSize, infoId, pagBtnsId) {
  ...
  function getAllRows() {
    return Array.from(tbody.querySelectorAll('tr')).filter(r => !qhseIsTablePlaceholderRow(r));
  }
